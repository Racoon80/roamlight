...
def _sso_href(next_url: str) -> str:
    ...
    ⚠ The joining is done here and not in the template: `next` has to be one of
      OUR paths, and a template is the wrong place to be deciding that.
    """
    link = _sso_link()
    if not link:
        return ""
...
def page_login(request: Request, next: str = "/", error: str = ""):
    ...
    #   to exist, because that is where the button lives and where
    #   `security._no_access` sends a browser.
...
def api_app_login(request: Request, body: dict = Body(...)):
    ...
    """
    if not config.auth_local():
        # ⚠ Say WHY, and say what to do instead. This was a bare "not found",
        #   and on a site that signs people in through a provider that is the
        #   answer every single person gets when they try the obvious thing --
        #   with nothing to tell them the road exists but runs elsewhere. It
        #   gives nothing away: the sign-in page says the same to anybody.
        raise HTTPException(
            status_code=404,
            detail="This site signs people in through single sign-on, so there "
                   "is no password to type here. Open the site in a browser, "
                   "sign in there, and press \u201cPhone & tablet\u201d \u2192 "
                   "\u201cShow the code\u201d; then scan or type that code.")
    ip = security.client_ip(request)
    if auth.blocked(ip):
